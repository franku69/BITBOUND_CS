...
( () => {
  ...
  function events(t, step) {
    ...
    const add = (n, span, type, gain) => out.push({
      freq: frequency(pitch(t, n)),
      dur: span * d,
      type,
      gain
    });
    if (degree !== null && !(ambient && bar % 4 === 3 && beat > 3)) add(degree, .78, t.lead, busy ? .058: .048);
    if (beat === 0 || (!ambient && (beat === 4 || (busy && [3, 6].includes(beat))))) add(chord - 14 + (beat === 6 ? 4: 0), busy ? 1.2: 2.8, 'triangle', busy ? .082: .058);
  ...
  class Player { constructor(ctx, output, { setTimer = (fn, ms) => setInterval(fn, ms), clearTimer = id => clearInterval(id) } = {}) {
      this.ctx = ctx;
      this.output = output;
      ...
      this.enabled = !!on;
      if (on) this.start(); else this.halt(.04);
    } setBackground(hidden) {
      this.background = !!hidden;
      if (hidden) this.halt(0); else this.start();
    } start() {
      ...
      try {
        ...
        this.next = this.ctx.currentTime + .025;
        this.pump();
        if (!this.failed) this.timer = this.setTimer( () => this.pump(), 40);
      } catch {
        this.fail();
      }
    } halt(fade = 0) {
      ...
      if (bus) {
        ...
        bus.gain.setTargetAtTime(.0001, now, Math.max(.005, fade / 4));
      }
      for (const voice of [... this.voices]) {
        if (voice.bus !== bus && fade > 0) continue;
        try {
          voice.source.stop(this.ctx.currentTime + fade + .005);
        } catch {}
      ...
      /* Optional music cannot block play. */
    } pump() {
      if (!this.enabled || this.background || !this.bus || this.ctx.state !== 'running') {
        this.halt(0);
        return;
      }
      ...
      const ctx = this.ctx, amp = ctx.createGain(), nodes = [amp];
      let source, dur = e.dur || .07;
      if (e.drum && e.drum !== 'kick') {
        ...
        filter.frequency.value = e.drum === 'hat' ? 6200: 1500;
        source.connect(filter);
        filter.connect(amp);
        nodes.push(filter);
        dur = e.drum === 'hat' ? .035: .11;
      } else {
        source = ctx.createOscillator();
        source.type = e.drum ? 'sine': e.type;
      ...
      nodes.push(source);
      const gain = e.type === 'square' ? e.gain * .60: e.gain;
      amp.gain.setValueAtTime(.0001, at);
      amp.gain.linearRampToValueAtTime(gain, at + .008);
      if (!e.drum) {
        amp.gain.linearRampToValueAtTime(gain * .72, at + dur * .20);
        amp.gain.setValueAtTime(gain * .72, at + dur * .65);
      }
      amp.gain.exponentialRampToValueAtTime(.0001, at + dur);
      amp.connect(this.bus);
      const v = { source, nodes, bus: this.bus };
      this.voices.add(v);
      source.onended = () => this.release(v);
      source.start(at);
      source.stop(at + dur + .01);
    } destroy() {
      this.halt(0);
      this.track = null;
      this.noise = null;
  ...
  function preference(value) {
    try {
      if (typeof value === 'boolean') sessionStorage.setItem('bitbound-music', value ? 'on': 'off');
      return sessionStorage.getItem('bitbound-music') !== 'off';
    } catch {
      return typeof value === 'boolean' ? value: true;
    }
  }
  window.BitboundMusic = Object.freeze({
