...
import {blankProgress,validateState} from './storage.js?v=8749b2c7cc4b';
...
export function parseSession(raw,ids){
  ...
  if(raw.version===1&&raw.workspaces&&!raw.format)raw={format:SAVE_FORMAT,version:1,python:raw,story:null};
  if(raw.version===1&&raw.solved&&!raw.format)raw={format:SAVE_FORMAT,version:1,python:blankProgress(),story:raw};
  ...
  return {format:SAVE_FORMAT,version:1,name:String(raw.name||python.name||raw.story?.team||'Explorer').slice(0,60),python,story:raw.story||null};
}
export function serializeSession(snapshot){
  const data={format:SAVE_FORMAT,version:1,savedAt:new Date().toISOString(),name:snapshot.name||snapshot.python?.name||snapshot.story?.team||'Explorer',python:snapshot.python||blankProgress(),story:snapshot.story||null};
