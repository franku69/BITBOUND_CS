...
self.onmessage = async ({data}) => {
  ...
  try {
    ...
    if (payload.mode === 'init') {self.postMessage({id, type:'ready'}); return;}
    ...
    for (const file of previousFiles) {try {py.FS.unlink('/home/pyodide/workspace/' + file);} catch { /* Already removed by code. */ }}
