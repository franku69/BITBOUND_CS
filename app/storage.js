...
export const blankProgress = () => ({version:1, name:'', active:'q01', completed:{}, workspaces:{}, hints:{}, attempts:{}, practiceSolution:{}});
export function validateState(raw, ids) {
  ...
  for (const id of [...ids,'free']) {
    ...
    if (workspace && typeof workspace === 'object') {
      ...
      if (Object.hasOwn(safe,'main.py')) next.workspaces[id] = {files:safe, current:Object.hasOwn(safe,workspace.current) ? workspace.current : 'main.py', stdin:String(workspace.stdin || '').slice(0,12000)};
    }
    if (ids.has(id)) {
      if (raw.completed?.[id]) next.completed[id] = {at:String(raw.completed[id].at || '').slice(0,50), solutionViewed:!!raw.completed[id].solutionViewed, code:typeof raw.completed[id].code === 'string' ? raw.completed[id].code.slice(0,MAX_FILE_BYTES) : ''};
...
export class ProgressStore {
  constructor(ids) { this.ids=ids; this.state=blankProgress(); this.revision=0; this.savedRevision=0; }
  changed() { this.revision++; }
  get dirty() { return this.revision!==this.savedRevision; }
  markSaved(revision=this.revision) { this.savedRevision=revision; }
  import(raw) {
    const validated=validateState(raw,this.ids);
    this.state=validated; this.changed(); this.markSaved();
  }
  reset() { this.state=blankProgress(); this.changed(); this.markSaved(); }
}
