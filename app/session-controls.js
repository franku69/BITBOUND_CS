...
import {MAX_SAVE_BYTES,parseSession,serializeSession} from './session-file.js?v=954f5e58019e';
export function installSessionControls({ids,getSnapshot,validate=pack=>pack,applySnapshot,onSaved=()=>{},hasUnsaved=()=>false,report=()=>{}}){
  ...
  const status=text=>{for(const node of document.querySelectorAll('[data-session-status]'))node.textContent=text;report(text);};
  const lock=value=>{busy=value;for(const button of buttons)button.disabled=value;};
  async function save(){
    if(busy)return;lock(true);
    try{
      const snapshot=await getSnapshot();const file=serializeSession(snapshot);
      download(file.name,file.text,'application/json');onSaved(snapshot);
  ...
  function load(){if(!busy)input.click();}
  input.onchange=async event=>{
    ...
    try{
      ...
      await applySnapshot(pack);status('Loaded your chosen save file. Further changes need a new Save file.');
  ...
  window.addEventListener('beforeunload',event=>{
    if(!leaving&&hasUnsaved()){event.preventDefault();event.returnValue='';}
  });
  // Back/forward cache must not silently restore the previous student's session.
  window.addEventListener('pageshow',event=>{if(event.persisted){leaving=true;window.location.reload();}});
  return {save,load,newPlayer,status};
}
