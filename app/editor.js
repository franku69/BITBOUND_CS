...
export class PythonEditor { constructor(textarea, onChange, onNotice) {
    ...
    textarea.addEventListener('input', () => {
      if (!this.cm) onChange(this.getValue());
    });
    textarea.addEventListener('keydown', event => {
      if (event.key === 'Tab' && !this.cm) {
    ...
    loadVendor().then(CM => {
      this.cm = CM.fromTextArea(textarea, {
        ...
        viewportMargin: 12,
        inputStyle: 'textarea',
        screenReaderLabel: 'Python source code editor',
        extraKeys: {
          Tab: cm => cm.somethingSelected() ? cm.indentSelection('add'): cm.replaceSelection('    ', 'end', '+input'),
          'Shift-Tab': cm => cm.indentSelection('subtract'),
          'Ctrl-Space': () => this.complete(),
          'Cmd-Space': () => this.complete(),
      ...
      for (const [key, value] of Object.entries({
        ...
        autocomplete: 'off',
        spellcheck: 'false'
      })) this.cm.getInputField().setAttribute(key, value);
      this.cm.on('change', () => {
        if (!this.muted) onChange(this.getValue());
      });
    ...
    if (this.cm) {
      this.cm.replaceSelection(value, 'end', '+input');
      this.cm.focus();
    } else {
      const t = this.textarea;
      t.setRangeText(value, t.selectionStart, t.selectionEnd, 'end');
      t.focus();
      this.onChange(t.value);
    }
  } indent() {
    if (this.cm) this.cm.indentSelection('add'); else this.insert('    ');
  } outdent() {
    if (this.cm) this.cm.indentSelection('subtract');
  } command(name) {
    if (this.cm) {
      this.cm.execCommand(name);
      this.cm.focus();
    }
  } complete() {
    if (!this.cm) return;
    ...
    this.cm.showHint({
      ...
      hint: cm => {
        ...
        return {
          list: [... names].filter(word => word.startsWith(prefix) && word !== prefix).sort().slice(0, 50),
    ...
    if (this.cm && this.traceLine !== null) this.cm.removeLineClass(this.traceLine, 'background', 'trace-highlight');
    this.traceLine = null;
  } highlight(line) {
    this.clearTrace();
    if (this.cm) {
      this.traceLine = Math.max(0, line - 1);
      this.cm.addLineClass(this.traceLine, 'background', 'trace-highlight');
      this.cm.scrollIntoView({ line: this.traceLine, ch: 0 }, 50);
    }
  } refresh() {
