...
export class PythonRunner { constructor(onStatus = () => {}, onStream = () => {}) {
    this.worker = null;
    this.sequence = 0;
    this.pending = null;
    this.loaded = false;
    ...
    this.worker.onmessage = ({ data }) => {
      ...
      if (data.type === 'error') {
        ...
        if (data.type === 'ready') this.loaded = true;
        pending.resolve(data.result);
      }
    };
    this.worker.onerror = (event) => {
      if (this.worker !== worker) return;
      this.stop('Python could not start. Check your connection or download the offline pack, then try again. ' + (event.message || ''));
    };
    ...
    this.warming = this.request({ mode: 'init' }, 240000).then( () => {
