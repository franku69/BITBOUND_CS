...
const STORY_SHOTS = Object.freeze({
  rock: { stage: 'trail', cast: [], action: 'rock' },
  birth: { stage: 'trail', cast: ['hero'], action: 'birth' },
  byte: { stage: 'trail', cast: ['hero', 'byte'] },
  trail: { stage: 'trail', cast: ['hero', 'byte'] },
  cage: { stage: 'cell', cast: ['hero', 'party'], action: 'cage' },
  rescue: { stage: 'cell', cast: ['hero', 'party'], action: 'rescue' },
  party: { stage: 'trail', cast: ['hero', 'party'] },
  camp: { stage: 'camp', cast: ['hero', 'party'], action: 'camp' },
  betrayal: { stage: 'camp', cast: ['hero', 'party'], action: 'betrayal' },
  prison: { stage: 'cell', cast: ['hero', 'byte'], action: 'prison' },
  escape: { stage: 'cell', cast: ['hero', 'byte'], action: 'escape' },
  arrival: { stage: 'hall', cast: ['hero', 'demon'], action: 'arrival' },
  reunion: {
    stage: 'hall',
    cast: ['hero', 'party', 'demon'],
    action: 'reunion',
    elite: true
  },
  ...
  victory: { stage: 'hall', cast: ['hero'], action: 'victory' },
  treasure: { stage: 'hall', cast: ['hero'], action: 'treasure' },
  impact: { stage: 'hall', cast: ['hero'], action: 'impact' },
  leap: { stage: 'hall', cast: ['hero'], action: 'leap' },
  collapse: { stage: 'trail', cast: ['hero'], action: 'collapse' },
  dawn: { stage: 'trail', cast: ['hero'], action: 'dawn' }
});
function paintDemon(g, fused, part = null) {
  ...
  for (let side = 0; side < 2; side++) {
    ...
    if (!part || part === 'wings') {
      ...
      storyLine(g, '#a46372', 1, [[35, 40], [18, 27], [26, 56], [15, 50]]);
      storyLine(g, '#74344f', 2, [[33, 44], [14, 44], [23, 58]]);
    }
    if (!part || part === 'arms') {
      p('#171724', [[31, 47], [23, 52], [19, 76], [25, 86], [34, 78], [38, 58]]);
      p('#8a566b', [[29, 52], [25, 61], [25, 74], [30, 78], [33, 60]]);
  ...
  for (let i = 0; i < 5; i++) {
    p(i % 2 ? '#78455d': '#493248', [[33 - i, 61 + i * 7], [47, 66 + i * 7], [62 + i, 61 + i * 7], [60 + i, 69 + i * 7], [47, 75 + i * 7], [35 - i, 69 + i * 7]]);
  }
  p('#ad7786', [[31, 37], [43, 41], [47, 52], [39, 57], [32, 50]]);
  p('#6b405d', [[49, 43], [63, 37], [64, 51], [56, 57], [49, 53]]);
  p(gold, [[46, 44], [53, 52], [48, 64], [41, 53]]);
  p('#fff0cb', [[46, 49], [49, 53], [47, 58], [44, 53]]);
  ...
  r('#fff1b6', 52, 22, 5, 1);
  r('#784453', 47, 27, 3, 2);
  storyLine(g, '#583043', 2, [[40, 29], [49, 31], [57, 28]]);
  r('#e5b2a0', 46, 31, 5, 1);
  r('#26283a', 28, 99, 13, 12);
  r('#26283a', 57, 99, 14, 12);
  r(gold, 26, 108, 17, 3);
  r(gold, 56, 108, 18, 3);
  if (fused) {
    // BYTE's second face emerges in the breastplate over Veyr's heart.
    p('#19383e', [[35, 44], [47, 38], [60, 44], [60, 60], [48, 69], [35, 61]]);
    p('#71a99a', [[38, 46], [48, 42], [57, 47], [56, 59], [48, 65], [39, 59]]);
    p('#213f40', [[37, 47], [38, 42], [42, 44], [44, 40], [48, 44], [52, 41], [57, 44], [58, 49], [52, 46], [49, 48], [45, 45], [41, 49]]);
    r('#122d39', 40, 49, 5, 6);
    r('#122d39', 50, 49, 4, 6);
    r('#fff1c2', 41, 49, 3, 2);
    r('#fff1c2', 51, 49, 2, 2);
    storyLine(g, '#163741', 2, [[40, 58], [47, 62], [54, 57]]);
    r('#d6f1cb', 44, 59, 7, 1);
    for (let i = 0; i < 3; i++) {
      storyLine(g, '#8bdccb', 1, [[29, 49 + i * 7], [33, 47 + i * 6], [36, 49 + i * 5]]);
      storyLine(g, '#8bdccb', 1, [[60, 49 + i * 5], [64, 47 + i * 6], [68, 49 + i * 7]]);
    }
  }
}
const storyBossFrames = {
  paladin: createStorySurface(62, 80, g => g.drawImage(partySprite(0, 'angry', true), 0, 0, 62, 80)),
  demon: createStorySurface(96, 112, g => paintDemon(g, false)),
  fused: createStorySurface(96, 112, g => paintDemon(g, true))
};
function drawPlotBoss(b, x, y) {
  const sprite = storyBossFrames[b.storyKind];
  ...
  if (b.storyKind === 'paladin') {
    ...
    drawRigActor(ctx, {
      ...
      foot: y + b.h,
      scale: (b.h + 10) / 94,
      face: player.x < b.x ? - 1: 1,
      motion,
      phase: (state.gameTime * 2) % 1,
      mood: 'angry',
      armed: true
    });
  } else drawDemonActor(ctx, {
    x: x + b.w / 2,
    foot: y + b.h,
    scale: (b.h + 14) / 112,
    boss: b.storyKind,
    motion: b.action || 'idle',
    phase: (state.gameTime * .75) % 1
  });
  if (b.storyKind === 'fused') {
    const open = b.action === 'recover';
    ctx.strokeStyle = open ? '#ffe58b': '#a6f0e8';
    ctx.lineWidth = 2;
    ctx.strokeRect(x - 5, y - 5, b.w + 10, b.h + 10);
    ctx.font = 'bold 9px monospace';
    ctx.textAlign = 'center';
    ctx.fillStyle = ctx.strokeStyle;
    ctx.fillText(open ? 'CROWN OPEN — STRIKE!': 'SHIELDED — DODGE', x + b.w / 2, y - 28);
  }
  return true;
}
function drawStoryRock(g, t, birth) {
  ...
  for (let side = 0; side < 2; side++) {
    ...
    storyPoly(g, '#243641', [[4, 261], [3, 79], [40, 63], [72, 89], [91, 126], [98, 204], [81, 264]]);
    storyPoly(g, '#6c817d', [[6, 87], [39, 69], [58, 98], [35, 126], [6, 136]]);
    storyPoly(g, '#475e60', [[7, 137], [37, 126], [75, 112], [88, 176], [78, 232], [8, 258]]);
    storyLine(g, '#a3afa0', 2, [[6, 88], [38, 72], [57, 98], [33, 128], [7, 136]]);
    storyLine(g, '#203641', 3, [[56, 144], [30, 168], [49, 208], [8, 235]]);
    g.restore();
  }
  storyRect(g, '#ffe4a9', - 2, 83 - smoothStory(t) * 22, 4, 181 + smoothStory(t) * 22);
  g.restore();
...
function drawCastleCollapse(g, t) {
  ...
  for (let row = 0; row < 5; row++) for (let col = 0; col < 7; col++) {
    ...
    g.save();
    g.translate(x + 21, y + 22);
    g.rotate(sign * amount * (row === 0 ? .9: .24));
    g.drawImage(castle, sx, sy, 43, 44, - 21, - 22, 43, 44);
    g.restore();
  }
  for (let i = 0; i < 30; i++) {
    const span = fall * (30 + (i * 47) % 211), x = 344 + (i % 2 ? 1: - 1) * span;
    g.globalAlpha = fall * (.18 + (i % 4) * .05);
    storyRect(g, i % 3 ? '#867080': '#d5b79c', x, 269 - (i % 6) * 7, 30 + i % 7 * 7, 10 + i % 5 * 4);
  }
  g.globalAlpha = 1;
}
function drawCellFront(g, gate) {
  ...
  for (let i = 0; i < 11; i++) {
    const x = 332 + i * 27;
    storyRect(g, '#0b1620', x, 52, 5, 211);
    storyRect(g, '#72848b', x + 1, 54, 1, 205);
  }
  g.save();
  g.beginPath();
  g.rect(205, 47, 420, 219);
  g.clip();
  const x = doorX + open * 116;
  for (let n = 0; n < 4; n++) {
    storyRect(g, '#0b1620', x + n * 31, 52, 6, 211);
    storyRect(g, '#9da5a2', x + n * 31 + 1, 54, 2, 207);
  }
  storyRect(g, '#536975', x - 1, 54, doorWidth, 5);
  storyRect(g, '#91a19f', x - 1, 54, doorWidth, 1);
  storyRect(g, '#536975', x - 1, 258, doorWidth, 5);
  storyRect(g, '#c1a772', x + 1, 201, 14, 21);
  storyRect(g, '#1a2935', x + 6, 208, 4, 7);
  g.restore();
  storyRect(g, '#172732', 205, 46, 420, 8);
  storyRect(g, '#8e9b98', 205, 46, 420, 2);
  storyRect(g, '#263b47', 616, 48, 9, 217);
  storyRect(g, '#697e85', 617, 49, 2, 214);
}
function drawSceneProp(g, p) {
  ...
  if (p.id === 'cupStone') {
    storyPoly(g, '#59514c', [[p.x - 18, p.y], [p.x + 45, p.y], [p.x + 48, p.y - 5], [p.x + 38, p.y - 7], [p.x - 13, p.y - 6]]);
    return;
  }
  if (p.id === 'rivet') {
    storyRect(g, '#b0aaa0', p.x - 2, p.y - 1, 5, 3);
    return;
  }
  if (p.id === 'brokenShield') {
    storyPoly(g, '#6d8994', [[p.x - 17, p.y], [p.x - 20, p.y - 18], [p.x - 5, p.y - 24], [p.x + 1, p.y - 12], [p.x - 5, p.y - 5]]);
    storyPoly(g, '#9d8f70', [[p.x + 3, p.y - 13], [p.x + 10, p.y - 20], [p.x + 17, p.y - 13], [p.x + 15, p.y], [p.x + 7, p.y - 1]]);
    return;
  }
  if (p.id === 'ribbon') {
    paintWristRibbon(g, p, { wrap: 0, tie: 0, loose: true });
    return;
  }
  if (p.id === 'pebble') {
    storyPoly(g, '#aa9685', [[p.x - 4, p.y], [p.x - 2, p.y - 4], [p.x + 3, p.y - 3], [p.x + 5, p.y], [p.x + 2, p.y + 3], [p.x - 3, p.y + 2]]);
    return;
  }
  if (p.id === 'hammer') {
    g.save();
    g.translate(p.x, p.y);
    g.rotate(p.angle);
    storyRect(g, '#98734e', - 2, - 2, 28, 4);
    storyRect(g, '#b8c1bd', 22, - 7, 10, 14);
    storyRect(g, '#e1dfcf', 22, - 7, 9, 2);
    g.restore();
    return;
  }
  if (p.id === 'fallenGear') {
  ...
  if (['scroll', 'key', 'map', 'snack'].includes(p.id)) {
    ...
    g.rotate(p.angle || 0);
    if (p.id === 'scroll') {
      storyRect(g, '#7a634b', - 17, - 16, 34, 32);
      storyRect(g, '#d8c494', - 15, - 15, 30, 28);
      storyRect(g, '#f1dfad', - 18, - 17, 36, 5);
      storyRect(g, '#b89b72', - 18, 12, 36, 4);
      for (let i = 0; i < 4; i++) storyRect(g, '#7a5d4a', - 10, - 8 + i * 4, 20 - i % 2 * 4, 1);
    }
    if (p.id === 'key') {
      g.strokeStyle = '#e1c184';
      g.lineWidth = 3;
      g.beginPath();
      g.arc(- 9, 0, 5, 0, Math.PI * 2);
      g.stroke();
      storyLine(g, '#dec58e', 3, [[- 4, 0], [16, 0], [16, 6], [11, 6], [11, 3]]);
    }
    if (p.id === 'map') {
      storyPoly(g, '#bda981', [[- 36, - 9], [33, - 9], [43, 3], [- 43, 3]]);
      storyLine(g, '#645641', 1, [[- 26, - 5], [- 15, - 2], [4, - 6], [26, - 2]]);
      for (let i = 0; i < 4; i++) storyRect(g, DAWN_COMPANY[i].color, - 26 + i * 17, - 6, 4, 4);
    }
    if (p.id === 'snack') {
      storyPoly(g, '#bc8152', [[- 8, - 5], [3, - 8], [10, - 3], [8, 5], [- 6, 6], [- 10, 1]]);
      storyLine(g, '#eed09a', 2, [[- 5, - 4], [- 2, 2], [1, - 5], [4, 2]]);
    }
    g.restore();
  } else {
    ...
    if (p.id === 'dagger' && p.temperature) {
      g.save();
      g.translate(p.x, p.y);
      g.rotate(angle);
      g.globalAlpha = p.temperature;
      storyLine(g, '#ffbb65', 2, [[0, - offset - 12], [0, - offset + 3]]);
      g.restore();
    }
  }
}
function drawSceneAtmosphere(g, f, front = false) {
  ...
  if (!front) {
    ...
    if (has('troll')) {
      ...
      storyRect(g, '#172a28', - 16, - 91, 9, 4);
      storyRect(g, '#f3edc9', - 13, - 90, 3, 2);
      storyRect(g, '#b8c097', 8, - 77, 6, 5);
      storyRect(g, '#314941', - 27, - 11, 19, 10);
      storyRect(g, '#314941', 7, - 11, 22, 10);
      g.restore();
    }
    if (has('victory') || has('treasure')) for (let i = 0; i < 35; i++) {
      const u = has('treasure') ? storyWindow(t, i % 5 * .055, .75): 1;
      storyRect(g, i % 3 ? '#c89656': '#f1d184', lerp(490, 260 + (i * 43) % 315, u), lerp(232, 260 - i % 5 * 4, u) - Math.sin(u * Math.PI) * 15, 7, 3);
    }
    if (has('treasure')) for (let i = 0; i < 4; i++) {
      const y = 221 - storyWindow(t, .2, .8) * 32 + Math.sin( (f.visualTime ?? t) * 7 + i) * 2;
      storyPoly(g, '#ecc789', [[433 + i * 29, y - 6], [439 + i * 29, y], [433 + i * 29, y + 6], [427 + i * 29, y]]);
    }
    return;
  }
  ...
  if (has('lockSpark')) for (let n = 0; n < 8; n++) {
    const p = t * 3 % 1;
    storyRect(g, '#d8b5f8', 216 + Math.cos(n * 2.4) * p * 24, 210 + Math.sin(n * 2.4) * p * 24, 2, 2);
  }
  if (has('gateRecall')) {
    // The held beam rests above the raised palms; crawling companions pass below.
    for (let n = 0; n < 8; n++) storyRect(g, '#536975', 244 + n * 17, 30, 4, 150);
    storyRect(g, '#9da9a8', 236, 180, 149, 8);
    for (let n = 0; n < 12; n++) storyRect(g, '#b9aa88', 247 + n * 11, 192 + (t * 80 + n * 7) % 39, 2, 2);
  }
  if (has('forge')) {
    const step = f.page.forgeStep || 0;
    if (step === 0) {
      g.globalAlpha = .25 + .15 * Math.sin(t * 20);
      storyRect(g, '#ffc675', 482, 210, 34, 14);
      g.globalAlpha = 1;
    }
    if (step === 1) {
    ...
    if (step === 2) for (let i = 0; i < 5; i++) {
      g.globalAlpha = .24;
      storyRect(g, '#c8d8d3', 501 + i * 4, 244 - (t * 38 + i * 11) % 30, 2, 6);
    }
    g.globalAlpha = 1;
  }
  if (has('crown') && (f.crownCharge ?? 1) > 0) for (const a of f.actors.values()) if (a.kind === 'ally') {
    const y = a.foot - 107;
    storyPoly(g, '#e8bce9', [[a.x - 12, y], [a.x - 10, y - 13], [a.x - 3, y - 5], [a.x + 2, y - 17], [a.x + 7, y - 5], [a.x + 13, y - 12], [a.x + 11, y]]);
  ...
  if (has('merging')) {
    drawArcaneBarrier(g, 544, 184, 75, t * 6, 0);
    for (let i = 0; i < 7; i++) storyLine(g, '#bef2df', 1, [[590, 190], [560, 157 + i * 9], [541, 155]]);
  }
  if (has('victory')) for (let i = 0; i < 16; i++) {
    const p = (t + i / 16) % 1;
    storyRect(g, '#ffe4a6', lerp(556, 382, p), lerp(185 + i % 5 * 8, 208, p) - Math.sin(p * Math.PI) * 22, 2, 4);
  }
...
function drawSceneDestruction(g, t, leap, frame) {
  ...
  const actor = frame.actors.get('hero'), strikeHand = actor && rigSocket({
    ... actor,
    motion: 'punch',
    phase: .48,
    fromMotion: null
  ...
  if (flash > 0) {
    g.save();
    g.globalAlpha = flash * .55;
    g.fillStyle = '#fff0c4';
    g.beginPath();
    g.ellipse(343, 262, 7 + flash * 17, 4 + flash * 9, 0, 0, Math.PI * 2);
    g.fill();
    g.restore();
  }
  if (!leap) {
    g.strokeStyle = '#eddab0';
    g.lineWidth = 2;
    g.beginPath();
    g.ellipse(343, 264, 1 + p * 150, 1 + p * 22, 0, 0, Math.PI * 2);
    g.stroke();
  }
  for (let i = 0; i < 24; i++) {
    const dx = (i * 53) % 204 - 102, y = 259 - p * (leap ? 210: 84) + p * p * ( (i * 73) % 159);
    g.save();
    g.translate(343 + dx * p * 2, y);
    g.rotate( (i % 2 ? 1: - 1) * p * 2);
    storyRect(g, i % 3 ? '#706474': '#baa08e', 0, 0, 4 + i % 6, 3 + i % 5);
    g.restore();
  }
  if (leap) for (let side = 0; side < 2; side++) {
    g.save();
    g.translate(320 + (side ? 1: - 1) * p * 165, - p * 38);
    g.rotate( (side ? 1: - 1) * p * .3);
    storyPoly(g, '#3c3044', side ? [[8, 0], [320, 0], [320, 35], [43, 28], [11, 43]]: [[- 320, 0], [- 8, 0], [- 15, 44], [- 61, 28], [- 320, 36]]);
    g.restore();
  }
  g.restore();
}
function renderStoryGraph(g, f) {
  ...
  g.drawImage(getStoryBackdrop(f.environment || f.stage), 0, 0);
  drawCinemaBack(g, f);
  drawSceneAtmosphere(g, f, false);
  f.rendered = [];
  ...
  for (const actor of actors) {
    ...
    g.save();
    g.globalAlpha = actor.opacity ?? 1;
    if (actor.dissolve > 0) {
      // Pixel breakup clips the single actor draw; no ghost body or extra sprite.
      g.beginPath();
      for (let row = 0; row < 24; row++) for (let col = 0; col < 16; col++) {
    ...
    g.beginPath();
    g.ellipse(actor.x, actor.ground ?? (actor.motion === 'jump' || actor.foot < 230 ? 264: actor.foot), actor.scale * 20, 3, 0, 0, Math.PI * 2);
    g.fill();
    if (actor.kind === 'boss') {
      drawDemonActor(g, { ... actor, phase: actor.actionPhase ?? ( (f.visualTime ?? f.t) * 2) % 1 });
    } else drawRigActor(g, { ... actor, precise: true });
    f.rendered.push(actor.id);
    g.restore();
  }
  for (const prop of f.props) {
    ...
    if (owner) {
      storyRect(g, rigPalette(owner).skin, point.x - 2, point.y - 2, 4, 3);
    }
  }
  drawSceneAtmosphere(g, f, true);
  drawCinemaProps(g, f);
  ...
  const caption = f.caption || f.location;
  if (caption) {
    storyRect(g, '#0a1429e8', 16, 18, Math.min(350, caption.length * 6.3 + 18), 24);
    g.fillStyle = '#ead6b0';
    g.font = '10px monospace';
    g.fillText(caption, 25, 34);
  }
}
function drawStoryPicture(g, art, t = 1, page = {}, ambientSeconds = 0) {
  ...
  renderStoryGraph(g, frame);
  storyRect(g, '#08101c', 0, 0, 640, 7);
  storyRect(g, '#08101c', 0, 293, 640, 7);
  storyRect(g, '#b5977855', 0, 7, 640, 1);
  g.restore();
  return frame;
}
...
function drawDemonActor(g, a) {
  ...
  g.save();
  g.translate(a.x - 48 * scale, a.foot - 112 * scale + (a.kneel || 0));
  g.scale(scale, scale * (1 - (a.kneel || 0) / (112 * scale)));
  for (const part of ['wings', 'arms']) for (let side = 0; side < 2; side++) {
    ...
    g.translate(pivot.x + (part === 'arms' ? (a.clawImpact || 0) * - 4: 0), pivot.y + (part === 'arms' ? (a.clawImpact || 0) * 24: 0));
    g.rotate(part === 'wings' ? - .06 + Math.sin(phase * Math.PI * 2) * .12: attack ? lerp(Math.sin(phase * Math.PI * 2) * .06, - .4 - Math.sin(phase * Math.PI) * .4, a.attackWeight ?? 1): Math.sin(phase * Math.PI * 2) * .06);
    g.drawImage(parts[part], 0, 0, 48, 112, - pivot.x, - pivot.y, 48, 112);
    g.restore();
  }
  g.drawImage(parts.body, 0, 0);
  if (a.fusionReveal > 0 && a.boss !== 'fused') {
    g.save();
    g.globalAlpha = a.fusionReveal;
    paintDemon(g, true, 'body');
    g.restore();
  }
  g.restore();
}
...
function drawStoryLight(g, f) {
  ...
  if (!storyLightCache.has(key)) storyLightCache.set(key, createStorySurface(640, 300, c => {
    ...
    shade.addColorStop(0, '#030b1740');
    shade.addColorStop(.58, '#06121c00');
    shade.addColorStop(1, '#07111b38');
    c.fillStyle = shade;
    c.fillRect(0, 0, 640, 300);
    const ray = c.createLinearGradient(0, 0, 420, 300);
    ray.addColorStop(0, warm ? '#ffe2a90d': '#9bc6ea0b');
    ray.addColorStop(1, '#00000000');
    c.fillStyle = ray;
    c.beginPath();
    c.moveTo(350, 0);
    c.lineTo(445, 0);
  ...
  for (let i = 0; i < 12; i++) {
    const x = 27 + (i * 127) % 610 + Math.sin( (f.visualTime ?? f.t) * 2 + i) * 5, y = 37 + ( (i * 47 - (f.visualTime ?? f.t) * 9) % 215 + 215) % 215;
    storyRect(g, warm ? '#ead3a428': '#abcddd24', x, y, 1, 2);
  }
}
