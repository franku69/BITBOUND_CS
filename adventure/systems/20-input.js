...
UI.introBtn.onclick = enterWorld;
UI.puzzleClose.onclick = () => {
  sfx.uiClose();
  hide(UI.puzzle);
};
UI.assessClose.onclick = () => {
  sfx.uiClose();
  hide(UI.assess);
};
UI.guideClose.onclick = () => {
  sfx.uiClose();
  hide(UI.guide);
};
...
UI.unstuckBtn.onclick = unstuckPlayer;
UI.respawnBtn.onclick = respawnPlayer;
UI.instructorClose.onclick = () => {
  sfx.uiClose();
  hide(UI.instructor);
};
...
UI.healBtn.onclick = () => {
  ...
  toast('Instructor: health restored.', '#7dff9b');
};
UI.coresBtn.onclick = () => {
  state.solved[state.level] = [true, true, true, true];
  updateCampaignHud();
  markStoryChanged();
  toast('Instructor: 4 cores granted.', '#7dff9b');
};
UI.assessmentBtn.onclick = () => openAssessment();
UI.bossBtn.onclick = () => {
  if (!state.boss || state.boss.dead) {
    state.assessmentPassed[state.level] = true;
    updateCampaignHud();
    spawnBoss();
  } else defeatBoss();
};
...
window.addEventListener('keydown', (e) => {
  ...
  if (plotUI.overlay.classList.contains('show')) {
    if (k === 'escape') {
      e.preventDefault();
      if (storyPlayback && !storyPlayback.paused) plotUI.still.onclick();
    }
    return;
  }
  if (UI.mentor.classList.contains('show')) {
    if (k === 'escape') {
      e.preventDefault();
      closeMentor();
    }
