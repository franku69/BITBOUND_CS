...
function addEncounter(col, type = window.BitboundEncounters.choose(state.level)) {
  ...
  state.enemies.push({
    ...
    w: spec.w,
    h: spec.h,
    vx: direction * spec.speed,
    vy: 0,
    hp: spec.hp,
    maxHp: spec.hp,
    alive: true,
    baseY: y,
    questionMob: !state.enemies.some(e => e.alive && e.questionMob && Math.floor(e.x / TILE / 40) === Math.floor(col / 40)),
    quizWarning: 0,
    t: Math.random() * 6,
    shoot: 2 + Math.random() * 2,
    hop: 1 + Math.random(),
    soundAt: 0
  });
  return true;
...
function mobIntent(e, intent, time) {
  e.intent = intent;
  e.intentTime = time;
}
function aimMob(e, p, seconds) {
  e.aim = { x: p.x, y: p.y };
  e.facing = Math.sign(p.x - e.x) || 1;
  mobIntent(e, 'windup', seconds);
}
function walkMob(e, dt) {
  const dx = e.vx * 60 * dt;
  e.x += dx;
  if (tileCollisionRect(e.x, e.y, e.w, e.h)) {
    e.x -= dx;
    e.vx *= - 1;
  }
  e.vy = Math.min(12, e.vy + GRAVITY * 60 * dt);
  const dy = e.vy * 60 * dt;
  e.y += dy;
  if (tileCollisionRect(e.x, e.y, e.w, e.h)) {
    e.y -= dy;
...
function moveEncounter(e, dt, p) {
  ...
  if (runeProtects(e)) {
    ...
    e.intent = 'patrol';
    e.vx = near ? Math.sign(dx) * Math.min(1.5, spec.speed): e.vx;
    if (fly) {
      const nx = e.x + e.vx * 60 * dt, ny = near ? lerp(e.y, p.y - e.h / 2, Math.min(1, dt * 2)): e.baseY + Math.sin(e.t * 2) * 12;
      if (!tileCollisionRect(nx, ny, e.w, e.h)) {
        e.x = nx;
        e.y = ny;
      }
    } else {
      if (e.grounded && near && tileCollisionRect(e.x + (e.vx > 0 ? e.w + 3: - 6), e.y, 4, e.h - 2)) e.vy = - 6;
      walkMob(e, dt);
    }
    return;
  }
  e.intent = e.intent || (e.type === 'mimic' ? 'sleep': 'patrol');
  e.intentTime = Math.max(0, (e.intentTime || 0) - dt);
  e.cooldown = Math.max(0, (e.cooldown ?? .7) - dt);
  if (e.intent === 'sleep') {
    e.vx = 0;
    if (near && Math.abs(dx) < 160) {
      aimMob(e, p, .65);
      mobCue(e);
    }
    ...
    if (e.intentTime <= 0) {
      ...
      if (e.type === 'bat') {
        ...
        if (e.type === 'wisp' && e.warp && !tileCollisionRect(e.warp.x, e.warp.y, e.w, e.h)) {
          e.x = e.warp.x;
          e.y = e.warp.y;
          e.baseY = e.y;
        }
        mobIntent(e, 'recover', .85);
        e.cooldown = e.type === 'wisp' ? 4: 2.4;
      } else if (e.type === 'crab' || e.type === 'crawler') {
        e.vx = e.facing * (e.type === 'crab' ? 2.1: 3.6);
        mobIntent(e, 'rush', e.type === 'crab' ? .28: .45);
      } else {
        e.vy = - (spec.jump || 6);
        e.vx = e.facing * (e.type === 'frog' ? 2.7: e.type === 'mimic' ? 2.2: 1.4);
        e.grounded = false;
        mobIntent(e, 'leap', .65);
      }
      mobCue(e);
    ...
    if (e.intentTime <= 0 || tileCollisionRect(nx, ny, e.w, e.h)) {
      e.vx = 0;
      e.vy = 0;
      e.baseY = e.y;
      mobIntent(e, 'recover', 1.2);
      e.cooldown = 2.4;
    } else {
      e.x = nx;
      e.y = ny;
    ...
    if (e.intentTime <= 0) {
      mobIntent(e, 'recover', e.type === 'frog' ? .65: .9);
      e.vx = 0;
      e.cooldown = .7;
    }
  } else if (e.intent === 'recover') {
    e.vx *= .85;
    if (e.intentTime <= 0) mobIntent(e, 'patrol', 0);
  } else {
    if (fly) {
