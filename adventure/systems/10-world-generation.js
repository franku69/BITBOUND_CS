...
function generateWorld(level) {
  ...
  for (let c = 0; c < COLS; c++) {
    if (c % 6 === 0) {
      const rr = randHash(c, level, seed);
      if (rr < .3) h--; else if (rr > .72) h++;
      h = clamp(h, 16, 22);
    }
    surface[c] = h;
  }
  ...
  for (const p of battlePlatforms) {
    for (let c = p.start; c < p.start + p.len; c++) put(c, arenaY - p.lift, Tile.PLATFORM);
  }
  // portal pedestal
  ...
  for (let c = 18; c < COLS - 20; c += 22) {
    const sy = surface[c];
    if (randHash(c, level, 8) > .35) {
      for (let x = c; x < c + 5; x++) put(x, sy - 4, Tile.PLATFORM);
    }
  }
  ...
  state.loot = [];
  for (let i = 0; i < 8; i++) {
    const c = 24 + i * 23 + (i % 2) * 4;
    if (c >= TERMINAL_COL - 8) continue;
    state.chests.push({
      x: c * TILE + 4,
      y: surface[c] * TILE - 22,
      w: 26,
  ...
  if (state.bossUnlocked && !state.bossDefeated[level]) spawnBoss(); else if (state.bossDefeated[level] && !ownsWeapon(bossWeaponId(level))) {
    const c = 205, sy = surfaceAt(c);
    spawnWeaponDrop(bossWeaponId(level), c * TILE, sy * TILE - 28, { guaranteed: true, source: 'BOSS DROP' });
  }
}
