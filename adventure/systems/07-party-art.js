...
function partySprite(index, pose = 'idle', elite = false) {
  index = clamp(index | 0, 0, 3);
  if (!PARTY_POSES.includes(pose)) pose = 'idle';
...
function paintAlly(g, index, pose = 'idle', elite = false, part = null) {
  if (!part) return paintRigActor(g, {
    ...
    phase: pose.startsWith('walk') ? (Number(pose.at(- 1)) || 0) / 4: .5,
    mood: pose,
    armed: false,
    precise: true,
  ...
  if (part === 'body') {
    ...
    if (index === 0) {
      ...
      p('#7c96a5', [[31, 38], [40, 35], [39, 45], [31, 50]]);
      line('#f5de9c', 2, [[20, 32], [30, 36], [42, 32]]);
      p(c, [[29, 38], [33, 38], [33, 43], [37, 43], [37, 46], [33, 46], [33, 50], [29, 50], [29, 46], [25, 46], [25, 43], [29, 43]]);
      for (let y = 54; y <= 64; y += 4) {
        r('#344955', 20, y, 23, 3);
        r('#92a6b1', 21, y, 21, 1);
      }
    } else if (index === 1) {
      p('#70518a', [[22, 40], [32, 45], [40, 39], [47, 79], [36, 83], [30, 79], [19, 82], [16, 76]]);
      line('#ceab72', 2, [[22, 41], [29, 49], [24, 79]]);
      line('#ceab72', 2, [[40, 40], [33, 48], [37, 79]]);
      r('#30273f', 28, 36, 7, 10);
      p('#d9d0ff', [[31, 35], [35, 40], [31, 44], [28, 40]]);
      ...
      p(shadow, [[23, 41], [39, 41], [41, 55], [22, 56]]);
      r(skin, 24, 42, 14, 10);
      r(light, 25, 42, 6, 4);
      line('#e3bf99', 1, [[33, 43], [30, 49], [34, 53]]);
      r('#31313c', 18, 55, 26, 6);
      r('#bba077', 29, 55, 7, 6);
      ...
      line('#ebd69b', 2, [[22, 34], [30, 41], [39, 34]]);
      r('#395f52', 25, 50, 13, 11);
      r('#edf0ce', 30, 52, 3, 7);
      r('#edf0ce', 28, 54, 7, 3);
      line('#395f52', 3, [[19, 34], [27, 56], [39, 68]]);
      r('#8b6444', 36, 64, 8, 10);
      r('#cfa877', 37, 65, 6, 2);
      r('#689e92', 19, 62, 5, 8);
      r('#c2fcda', 20, 62, 3, 4);
  ...
  if (elite && part === 'body') {
    for (const x of [13, 43]) {
      p('#d5b26b', [[x - 3, 30], [x + 1, 23], [x + 5, 28], [x + 8, 26], [x + 9, 36], [x, 36]]);
      r('#fff0b8', x, 30, 6, 2);
    }
    line('#e1bb76', 2, [[22, 64], [32, 67], [41, 63]]);
    r('#ab6fde', 29, 62, 5, 5);
    r('#392f42', 5, 65, 12, 10);
    r('#e3bc79', 6, 66, 10, 2);
    r('#edcf96', 10, 69, 3, 3);
    for (let i = 0; i < 3; i++) {
      r('#b7985e', 42 + i * 3, 69 + i % 2 * 2, 3, 6);
      r('#f5dba0', 42 + i * 3, 69 + i % 2 * 2, 2, 2);
    }
  }
  g.restore();
}
