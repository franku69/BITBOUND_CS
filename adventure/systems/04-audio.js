...
const AudioEngine = {
  ...
  musicTrack: 'setup',
  musicPhase: null,
  musicEnabled: window.BitboundMusic.preference(),
  sfxEnabled: true,
  ducked: true,
  background: false,
  ...
  dialogueOpen: false,
  dialogueVoice: null,
  dialogueFailed: false,
  dialogueSpeaker: 'BYTE',
  dialogueVoices: Object.create(null),
  init() {
    ...
    if (!this.ctx) {
      try {
        ...
        this.ctx.addEventListener?. ('statechange', () => {
          if (this.ctx.state === 'running' && !this.background && !document.hidden) this.musicPlayer.start();
        });
        this.refreshMix();
      } catch (e) {
        return null;
    ...
    if ( (this.ctx.state === 'suspended' || this.ctx.state === 'interrupted') && !this.resumePending) {
      ...
      Promise.resolve(this.ctx.resume()).catch( () => {}).finally( () => {
        this.resumePending = false;
        if (!this.background && !document.hidden) this.musicPlayer?.start();
      });
    }
  ...
  tone({
    ...
    amp.gain.setValueAtTime(Math.max(.0001, gain), begin);
    amp.gain.exponentialRampToValueAtTime(.0001, begin + dur);
    this.voices.add(osc);
    osc.onended = () => {
      osc.disconnect();
      amp.disconnect();
    ...
    osc.connect(amp);
    amp.connect(bus === 'music' ? this.musicBus: this.sfxBus);
    osc.start(begin);
    osc.stop(begin + dur + .01);
  },
  sequence(seq = []) {
    if (!this.sfxEnabled || !this.init()) return;
    let offset = 0;
    for (const t of seq) {
      this.tone({ ... t, start: offset }, 'sfx');
      offset += t.wait ?? t.dur ?? .05;
    }
    if (this.ducked) this.scheduleSuspend();
  },
  theme() {
    return window.BitboundMusic.catalog[this.musicTrack];
  },
  tick() {
    this.musicPlayer?.pump();
  },
  pauseMusic() {
    this.musicPlayer?.halt(.04);
  },
  syncMusic() {
  ...
  playWorld(level = 0) {
    this.level = level;
    this.musicMode = 'world';
    this.musicRequested = true;
    this.syncMusic();
  },
  playBoss(level = 0, phase = state.boss?.storyKind) {
    this.level = level;
    this.musicMode = 'boss';
  ...
  scheduleSuspend() {
    ...
    this.suspendTimer = setTimeout( () => {
      this.suspendTimer = null;
      if (this.ducked || this.background) this.suspend();
    }, 1200);
  },
  toggleMusic() {
    this.musicEnabled = !this.musicEnabled;
    window.BitboundMusic.preference(this.musicEnabled);
    this.syncMusic();
    if (!this.musicEnabled && this.ducked) this.scheduleSuspend();
    this.refreshMix();
    return this.musicEnabled;
  },
  setDialogue(open) {
    if (open) this.setSpeaker('BYTE'); else {
  ...
  setSpeaker(speaker) {
    ...
    if (this.dialogueSpeaker !== speaker) {
      this.dialogueVoice?.stop();
      this.dialogueSpeaker = speaker;
      this.dialogueVoice = this.dialogueVoices[speaker] || null;
    }
  ...
  syncDialogue() {
    ...
    if (!speak) {
      this.dialogueVoice?.stop();
      if (this.ducked && !this.background) this.scheduleSuspend();
      return;
    }
    try {
      ...
      this.cancelSuspend();
      this.dialogueVoice.play(context, this.sfxBus);
    } catch (error) {
      this.dialogueFailed = true;
      this.dialogueVoice?.stop();
  ...
  setBackground(value) {
    ...
    this.musicPlayer?.setBackground(this.background);
    if (this.background) this.suspend(); else {
      if (this.ctx) this.init();
      this.syncMusic();
      if (this.dialogueOpen) this.syncDialogue();
    }
    this.refreshMix();
  }
};
...
function updateAudioButtons() {
  for (const button of document.querySelectorAll('[data-story-music]')) {
    ...
    button.setAttribute('aria-pressed', String(AudioEngine.musicEnabled));
    button.title = AudioEngine.theme()?.name || 'Music';
  }
  if (UI.musicToggle) {
    UI.musicToggle.title = AudioEngine.theme()?.name || 'Music';
    UI.musicToggle.setAttribute('aria-pressed', String(AudioEngine.musicEnabled));
    UI.musicToggle.dataset.musicTrack = AudioEngine.musicTrack;
    domWrites.text(UI.musicToggle, AudioEngine.musicEnabled ? '♫ MUSIC ON': '♫ MUSIC OFF');
    domWrites.toggle(UI.musicToggle, 'off', !AudioEngine.musicEnabled);
  }
  if (UI.sfxToggle) {
    domWrites.text(UI.sfxToggle, AudioEngine.sfxEnabled ? '✦ SFX ON': '✦ SFX OFF');
    domWrites.toggle(UI.sfxToggle, 'off', !AudioEngine.sfxEnabled);
  }
  const storySound = $('storySound');
  if (storySound) {
    domWrites.text(storySound, AudioEngine.sfxEnabled ? 'Sound: on': 'Sound: off');
    storySound.setAttribute('aria-pressed', String(AudioEngine.sfxEnabled));
  }
  const dialogueSound = $('mentorSound');
  if (dialogueSound) {
    domWrites.text(dialogueSound, AudioEngine.sfxEnabled ? 'Sound effects: on': 'Sound effects: off');
    dialogueSound.setAttribute('aria-pressed', String(AudioEngine.sfxEnabled));
  }
}
