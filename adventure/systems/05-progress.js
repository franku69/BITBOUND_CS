...
function elapsedPrecise() {
  return Math.max(0, state.elapsedBase + (state.started && state.raceStarted ? (performance.now() - state.startPerf) / 1000: 0));
}
function elapsed() {
  return Math.floor(elapsedPrecise());
}
function timeFmt(sec) {
  sec = Math.max(0, Math.floor(sec));
  return `${String(Math.floor(sec/60)).padStart(2,'0')}:${String(sec%60).padStart(2,'0')}`;
}
function raceTimeFmt(sec) {
  sec = Math.max(0, Number(sec) || 0);
  const minutes = Math.floor(sec / 60), seconds = Math.floor(sec % 60), tenths = Math.floor( (sec - Math.floor(sec)) * 10);
  return `${String(minutes).padStart(2,'0')}:${String(seconds).padStart(2,'0')}.${tenths}`;
}
function updateRaceTimerDisplay() {
  if (UI.hudTime && state.raceStarted) domWrites.text(UI.hudTime, raceTimeFmt(elapsedPrecise()));
}
function allCores() {
  return state.solved[state.level].every(Boolean);
}
...
function hide(el) {
  if (el === UI.mentor || el.id === 'storyOverlay') AudioEngine.setDialogue(false);
  if (el.id === 'storyOverlay') stopStoryAnimation();
  resetTouchInput();
  if (el === UI.puzzle) closePractice();
  el.classList.remove('show');
  el.hidden = true;
  handheld?.refresh();
  syncPresentationPause();
}
function toast(msg, color = '#d9edff') {
...
function completedQuestionCount() {
  ...
  for (let level = 0; level < worlds.length; level++) {
    if (state.assessmentPassed[level]) completed += 2; else for (const passed of state.terminalSolved[level]) if (passed) completed++;
  }
...
function updateCampaignHud() {
  ...
  state.academicPoints = completed;
  updateAcademicHud();
...
function showKnowledgeReward(title, bonus) {
  ...
  UI.rewardBanner.classList.add('show');
...
function recordCorrectAnswers(count, title) {
  state.answerStreak += count;
  state.bestStreak = Math.max(state.bestStreak, state.answerStreak);
  const bonus = Math.min(50, Math.max(0, state.answerStreak - 1) * 5);
  state.score += bonus;
  ...
  updateCampaignHud();
  showKnowledgeReward(title, bonus);
}
function resetKnowledgeChain() {
...
function academicAttemptStatus() {
  return 'Practice freely. Retrying never deducts learning points';
}
function registerWrongSubmission() {
  state.wrong++;
  return { deducted: false, text: 'Use a hint and retry. No learning points deducted.' };
}
function worldData() {
  return worlds[state.level];
}
function currentRunner() {
  return state.members[CLASSROOM_RACE.pilotMemberIndex] || 'Explorer';
}
function surfaceAt(col) {
  return state.world?.surface[clamp(Math.floor(col), 0, COLS - 1)] ?? 20;
}
...
function getTile(c, r) {
  if (c < 0 || c >= COLS || r < 0 || r >= ROWS) return Tile.STONE;
  return state.world.tiles[r][c];
}
function isSolid(c, r) {
  return SOLID.has(getTile(c, r));
}
...
function onScreen(x, y, w = 0, h = 0, pad = 96) {
  return x + w >= state.camera.x - pad && x <= state.camera.x + W + pad && y + h >= state.camera.y - pad && y <= state.camera.y + H + pad;
}
function currentPet() {
  return PETS[state.petId] || PETS[DEFAULT_PET_ID];
}
function selectPet(id) {
  if (!PETS[id]) return false;
  state.petId = id;
  for (const option of UI.petOptions) {
    const selected = option.dataset.pet === id;
...
function updatePetHud() {
  const companion = currentPet();
  domWrites.text(UI.petHudIcon, companion.icon);
  domWrites.text(UI.petHudName, companion.name);
  domWrites.text(UI.petHudPerk, petSupport.label || companion.perk);
}
