...
const partyActors = DAWN_COMPANY.map( (member, index) => ({
  ... member,
  index,
  x: 0,
  y: 0,
...
function partyActive() {
  return plotState().partyFreed && !plotState().betrayed && state.level < 4;
}
function resetParty() {
  for (const a of partyActors) {
    Object.assign(a, {
      x: player.x - 40 - a.index * 30,
      vx: 0,
      vy: 0,
      onGround: true,
      motion: 'idle',
      phase: 0,
      actionTime: 0,
      cooldown: .7 + a.index * .3,
      heal: 0,
      attack: null
    });
    a.y = surfaceAt(clamp(Math.floor(a.x / TILE), 0, COLS - 1)) * TILE - 48;
  }
}
function partyTarget(a) {
  const b = state.boss;
  if (b && b.active && !b.dead && Math.abs(b.x - player.x) < 650) return b;
  let target = null, best = 420 * 420;
  for (const e of state.enemies) {
    if (!e.alive || e.questionMob || Math.abs(e.x - player.x) > 400) continue;
...
function movePartyActor(a, target, dt) {
  ...
  if (Math.abs(distance) > W && !onScreen(a.x, a.y, 32, 48)) {
    a.x = target;
    a.y = surfaceAt(clamp(Math.floor(a.x / TILE), 0, COLS - 1)) * TILE - 48;
    a.vy = 0;
  }
  ...
  if (a.onGround && (rise > 4 || blocked)) {
    a.vy = - Math.min(570, Math.sqrt(2 * 1050 * (Math.max(rise, blocked ? 64: 0) + 24)));
    a.onGround = false;
  }
...
function releasePartyAttack(a, action) {
  ...
  if (action.heal) {
    if (player.health < MAX_HEALTH) {
      ...
      a.heal = 10;
      burst(player.x + 12, player.y + 22, '#a6e0b3', 9);
  ...
  if (a.index === 1 || a.index === 3) {
    ...
    const box = {
      x: a.face > 0 ? a.x + 16: a.x + 16 - role.reach,
      y: a.y - 4,
      w: role.reach,
      h: 56
    };
    for (const e of state.enemies) if (e.alive && !e.questionMob && rects(box, e)) {
      e.hp -= role.damage;
      e.vx += a.face * 1.5;
      burst(e.x, e.y + 15, a.color, 6);
      if (e.hp <= 0) killEnemy(e);
    }
    if (state.boss && rects(box, state.boss)) applyBossDamage(state.boss, role.damage, a.face * .35, a.color);
    if (a.index === 0 && Math.abs(player.x - a.x) < 190) {
      player.invuln = Math.max(player.invuln, .35);
      burst(player.x + 12, player.y + 18, a.color, 4);
    }
    sfx.swing();
  }
}
function updateParty(dt) {
  if (!partyActive()) {
    for (const a of partyActors) a.attack = null;
    return;
  }
  for (const a of partyActors) {
    ...
    if (target) {
      const side = Math.sign(a.x - target.x) || - 1;
      destination = target.x + side * (role.range - 12) - 16;
    }
    movePartyActor(a, destination, dt);
    a.cooldown = Math.max(0, a.cooldown - dt);
    a.heal = Math.max(0, a.heal - dt);
    if (a.attack) {
      const action = a.attack;
      action.elapsed = Math.min(role.duration, action.elapsed + dt);
      a.phase = action.elapsed / role.duration;
      a.motion = role.motion;
      a.actionTime = role.duration - action.elapsed;
      if (!action.released && a.phase >= role.release) {
        action.released = true;
        releasePartyAttack(a, action);
      }
      if (action.elapsed >= role.duration) {
        a.attack = null;
        a.cooldown = role.cooldown;
        a.actionTime = 0;
      }
      continue;
    }
...
function drawPartyCombatEffect(a) {
  if (!a.attack) return;
  const p = a.phase, origin = rigSocket(partyRig(a));
  ctx.save();
  if (a.index === 0 || a.index === 2) {
    if (p > .38 && p < .7) {
      ctx.globalAlpha = Math.sin( (p - .38) / .32 * Math.PI) * .75;
      ctx.strokeStyle = a.color;
      ctx.lineWidth = 2;
      ctx.translate(origin.x, origin.y);
      ctx.scale(a.face, 1);
      ctx.beginPath();
      ctx.arc(0, 0, a.index === 2 ? 31: 24, - 1.7 + (p - .38) * 3, .15 + (p - .38) * 3);
      ctx.stroke();
    }
  } else if (p < .48) {
    const tip = partySpellOrigin(a);
    ctx.strokeStyle = a.color;
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.arc(tip.x, tip.y, 2 + p * 10, 0, Math.PI * 2);
    ctx.stroke();
  }
  ctx.restore();
}
