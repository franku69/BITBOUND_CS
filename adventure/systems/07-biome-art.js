...
function paintBiome(g, kind) {
  ...
  const palettes = {
    valley: ['#436783', '#9ac1be', '#d6d6a0'],
    forest: ['#101d32', '#274a53', '#7b9481'],
    cavern: ['#101622', '#243540', '#46696b'],
    dungeon: ['#101621', '#32303b', '#795757'],
    catacombs: ['#0c1420', '#273340', '#496570'],
    approach: ['#201d35', '#713b54', '#c78674']
  };
  const sky = palettes[kind] || palettes.valley, grad = g.createLinearGradient(0, 0, 0, 300);
  grad.addColorStop(0, sky[0]);
  grad.addColorStop(.67, sky[1]);
  ...
  if (kind === 'valley' || kind === 'forest' || kind === 'approach') {
    ...
    if (kind === 'valley') {
      for (const [x, y, w, h] of [[40, 28, 120, 8], [140, 50, 78, 5], [287, 25, 99, 7]]) {
        r('#dae8d688', x, y, w, h);
        r('#e8ecdf88', x + 15, y - 4, w * .52, 4);
      }
      ...
      r('#84bec3', 348, 124, 14, 123);
      r('#d2efde', 349, 128, 3, 115);
      r('#446778', 357, 128, 3, 115);
      ...
      for (let i = 0; i < 9; i++) {
        const x = i * 81 - 15, h = 21 + (i * 13) % 27;
        r('#4f5742', x + 9, 240 - h, 5, h);
        for (let j = 0; j < 3; j++) p(i % 2 ? '#426751': '#577c54', [[x - 7, 233 - j * 9], [x + 11, 216 - h + j * 2], [x + 29, 233 - j * 9]]);
      }
    }
    if (kind === 'forest') {
      for (let layer = 0; layer < 2; layer++) for (let i = 0; i < 10; i++) {
        ...
        line(layer ? '#31483e': '#345750', 3, [[x + 9, top + 10], [x - 15, top - 3], [x - 27, top - 20]]);
        line('#2c4643', 3, [[x + 11, top + 53], [x + 38, top + 25], [x + 50, top + 16]]);
      ...
      for (const x of [54, 209, 463, 581]) {
        r('#77574a', x, 237, 4, 22);
        p('#6d8c8d', [[x - 9, 238], [x - 7, 230], [x, 225], [x + 9, 231], [x + 11, 238]]);
        r('#b9d4b6', x - 4, 232, 2, 2);
        r('#b9d4b6', x + 4, 234, 3, 2);
      }
    }
    if (kind === 'approach') {
      ...
      for (const x of [55, 553]) {
        r('#302a3c', x, 109, 32, 137);
        r('#665060', x, 112, 4, 130);
        for (let j = 0; j < 3; j++) r('#292637', x - 2 + j * 14, 100, 9, 14);
        r('#111b2a', x + 10, 140, 12, 21);
        line('#957866', 1, [[x + 8, 189], [x + 21, 209], [x + 15, 225]]);
      }
      r('#1b1d2c', 225, 248, 220, 52);
      for (let i = 0; i < 15; i++) {
        r('#695152', 223 + i * 15, 245, 12, 5);
        r('#a38a70', 224 + i * 15, 245, 10, 1);
      }
    }
  } else if (kind === 'cavern') {
    ...
    for (const x of [88, 292, 455, 567]) {
      for (let j = 0; j < 3; j++) {
        const y = 231 - j % 2 * 14, xx = x + j * 11;
        p('#345e73', [[xx, y], [xx - 5, y - 26], [xx, y - 43], [xx + 7, y - 23], [xx + 5, y]]);
        p(j % 2 ? '#75c5c9': '#ad91cc', [[xx, y - 3], [xx - 2, y - 27], [xx, y - 37], [xx + 4, y - 22]]);
        r('#d5eddf', xx, y - 26, 1, 13);
      }
    }
    ...
    for (let row = 0; row < 14; row++) for (let col = 0; col < 17; col++) {
      const x = col * 43 - row % 2 * 21, y = row * 20;
      r(cat ? '#35444c': '#3e3b46', x, y, 40, 18);
      r(cat ? '#59707155': '#9c7e6855', x, y, 40, 1);
      if ( (row * 7 + col) % 13 === 0) line('#171e2b', 1, [[x + 9, y], [x + 14, y + 7], [x + 8, y + 15]]);
    }
    for (let i = 0; i < 5; i++) {
      const x = 16 + i * 137;
      arch(g, x, 56, 98, 207, cat ? '#6b7471': '#7c6970');
      arch(g, x + 6, 65, 86, 198, '#101b29');
      if (cat) {
        for (let row = 0; row < 4; row++) {
          r('#43505a', x + 13, 112 + row * 33, 74, 4);
          for (let j = 0; j < 4; j++) {
            r('#b2af92', x + 18 + j * 16, 96 + row * 33, 9, 11);
            r('#202d38', x + 19 + j * 16, 100 + row * 33, 2, 3);
            r('#202d38', x + 23 + j * 16, 100 + row * 33, 2, 3);
          }
        }
      } else for (let j = 0; j < 6; j++) {
        r('#495863', x + 15 + j * 12, 102, 4, 160);
        r('#869092', x + 15 + j * 12, 102, 1, 154);
      }
      r('#29323e', x - 8, 42, 8, 222);
      r('#a99a7b', x - 9, 50, 12, 3);
      r('#56605e', x - 11, 257, 15, 7);
      r('#3c313a', x + 96, 133, 8, 36);
      r('#c99565', x + 98, 139, 4, 17);
      r(cat ? '#8ccecb': '#f7bb77', x + 97, 127, 6, 16);
      r('#f5eac2', x + 99, 129, 2, 9);
    }
    if (cat) {
      r('#1b2936', 0, 265, 640, 35);
      for (let i = 0; i < 14; i++) r('#435462', i * 51, 272 + i % 3 * 8, 40, 2);
    }
  }
  ...
  for (let i = 0; i < 48; i++) {
    r('#07111c33', (i * 137) % 640, (i * 61) % 280, 2, 2);
  }
}
function drawRegionalBackdrop(level) {
  const kind = REGION_BIOMES[level], background = getStoryBackdrop(kind);
  ...
  if (PERF.richFx && level < 6 && !mentorMotionQuery.matches) {
    const t = state.gameTime, count = level === 1 ? 10: 6;
    ctx.fillStyle = level === 1 ? '#c5d99577': level === 2 ? '#9fe8e866': '#e2d6b044';
    for (let i = 0; i < count; i++) {
      const x = ( (i * 173 + t * (level === 0 ? 5: 1.8)) % (W + 20)) - 10, y = H * (.21 + (i % 5) * .085) + Math.sin(t * .8 + i) * 5;
...
function drawRegionCrystal(x, y, w) {
  for (let i = 0; i < 3; i++) {
    const h = 14 + i * 7;
    storyPoly(ctx, '#142b39', [[x + i * 9, y], [x + i * 9 - 2, y - h], [x + i * 9 + 3, y - h - 9], [x + i * 9 + 8, y - h], [x + i * 9 + 9, y]]);
    storyPoly(ctx, w.accent, [[x + i * 9 + 1, y - 2], [x + i * 9 + 1, y - h], [x + i * 9 + 3, y - h - 6], [x + i * 9 + 4, y - 2]]);
  }
}
