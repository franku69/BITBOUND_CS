...
for path in [ROOT/'index.html',ROOT/'story.html',ROOT/'adventure/index.html',ROOT/'app/lab.html',ROOT/'app/workspace.html']:
    References(path).feed(path.read_text())
for path in ROOT.rglob('*.js'):
    if path.name=='sw-template.js' or 'systems' in path.parts or 'vendor' in path.parts or 'runtime' in path.parts:continue
...
assert manifest['version'] in (ROOT/'sw.js').read_text()
assert 'team-grid' not in (ROOT/'index.html').read_text()
assert 'DSA MASTERY' not in (ROOT/'index.html').read_text()
assert 'location.replace' not in (ROOT/'app/lab.html').read_text()
assert 'pythonLabBtn' not in (ROOT/'story.html').read_text()
assert 'id="game"' not in (ROOT/'index.html').read_text()
assert 'adventure/game.js' not in (ROOT/'app/lab.html').read_text()
assert not re.search('runMiniPython|evalMiniPythonExpr|numericOutput', (ROOT/'adventure/game.js').read_text())
# Every hardcoded DOM reference in the main controller must exist in the page.
ids=set(re.findall(r'id="([^"]+)"',(ROOT/'app/lab.html').read_text()))
for name in re.findall(r"\$\('([^']+)'\)",(ROOT/'app/main.js').read_text()):
    if name not in ids:errors.append('Missing UI element '+name)
# Story controls must exist too; a permissive test DOM cannot hide missing IDs.
ids=set(re.findall(r'id="([^"]+)"',(ROOT/'story.html').read_text()))
for name in re.findall(r"\$\('([^']+)'\)",(ROOT/'adventure/game.js').read_text()):
